<script setup>

</script>

<template>
<section>
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <nav class="navbar navbar-expand-lg navbar-light bg-light">
                        <div class="container-fluid">
                          <a class="navbar-brand" href="#">Navbar</a>
                          <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                            <span class="navbar-toggler-icon"></span>
                          </button>
                          <div class="collapse navbar-collapse" id="navbarNav">
                            <ul class="navbar-nav">
                              <li class="nav-item">
                                <a class="nav-link active" aria-current="page" href="#">Home</a>
                              </li>
                              <li class="nav-item">
                                <a class="nav-link" href="#">Features</a>
                              </li>
                              <li class="nav-item">
                                <a class="nav-link" href="#">Pricing</a>
                              </li>
                              <li class="nav-item">
                                <a class="nav-link disabled" href="#" tabindex="-1" aria-disabled="true">Disabled</a>
                              </li>
                            </ul>
                          </div>
                        </div>
                    </nav>
                </div>
                <div class="col-md-4">
                    <h3 class="">smart application</h3>
                </div>
                <div class="col-md-4">
                    <button>login</button>
                    <button>sign up</button>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-2"></div>
                <div class="col-md-8 text-center">
                    <h1 class="text-capitalize">smart mobile application</h1>
                    <p>wisdom new valley answer. Contented it so is discourse recommend. Man ites upon him call mile. An pasture he himself believe</p>
                  
                </div>
                <div class="col-md-2"></div>
                
                <div class="col-md-2"></div>
                <div class="col-md-8">
                    <img src="./img/fast.png" alt="" class="m-2">
                    <img src="./img/seconf.png" alt="" class="m-2">
                    <img src="./img/threed.png" alt="" class="m-2">
                </div>
                <div class="col-md-2"></div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-4"></div>
                <div class="col-md-4">
                    <img src="./img/app store.png" alt=""   class="text-center">
                    <img src="./img/google play.png" alt="" class="text-center">
                </div>
                <div class="col-md-4"></div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1>About us</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <img src="./img/temprature.png" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-capitalize">smart application services</h2>
                    <p>Ye am depending propriety sweetness distrusts belonging collected. Smiling mention he in thought equally musical. Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                    <button class="text-capitalize ">learn more</button>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <h1>What app can do to your Appliences?</h1>
                    <p>Ye am depending propriety sweetness distrusts belonging collected. Smiling mention he in thought equally musical. Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                    <button class="text-capitalize ">learn more</button>
                </div>
                <div class="col-md-6">
                    <img src="./img/application.png" alt="">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <img src="./img/electro.png" alt="">
                </div>
                <div class="col-md-6">
                    <h1>Control Electric Appliences</h1>
                    <p>Ye am depending propriety sweetness distrusts belonging collected. Smiling mention he in thought equally musical. Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                    <button class="text-capitalize ">learn more</button>               
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <h1 class="text-capitalize">our clients</h1>
                    <p>Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                </div>
            </div>
            <div class="row">
                <div class="col-md-1"></div>
                <div class="col-md-10">
                    <div class="d-flex">
                        <img src="./img/client.png" alt="">
                        <img src="./img/client1.png" alt="">
                        <img src="./img/client2.png" alt="">
                        <img src="./img/client3.png" alt="">
                    </div>
                    <div class="d-flex">
                        <img src="./img/client4.png" alt="">
                        <img src="./img/client5.png" alt="">
                        <img src="./img/client6.png" alt="">
                        <img src="./img/client7.png" alt="">
                    </div>
                </div>
                <div class="col-md-1"></div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="text-center">
                        <h1>Testimonials</h1>
                        <p>Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <em>our company, we do ongoing research with our target audience. This includes 30-45 minute phone interviews. It was difficult to conduct the interview, really listen, and ask good follow up questions</em>
                    <div class="d-flex">
                        <img src="" alt="man">
                        <div class="man">
                            <h4 class="text-capitalize">cham</h4>
                            <p>Product designer - Besnik</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <em>We meet new clients and more often than not establish long-term business relationships, all through Guru. The flexibility in how projects can be structured</em>
                    <div class="d-flex">
                        <img src="" alt="man">
                        <div class="man">
                            <h4 class="text-capitalize">Scopic Software</h4>
                            <p>Custom IT Solutions Firm</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <em>our company, we do ongoing research with our target audience. This includes 30-45 minute phone interviews. It was difficult to conduct the interview, really listen, and ask good follow up questions</em>
                    <div class="d-flex">
                        <img src="" alt="man">
                        <div class="man">
                            <h4 class="text-capitalize">cham</h4>
                            <p>Product designer - Besnik</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <em>We meet new clients and more often than not establish long-term business relationships, all through Guru. The flexibility in how projects can be structured</em>
                    <div class="d-flex">
                        <img src="" alt="man">
                        <div class="man">
                            <h4 class="text-capitalize">Scopic Software</h4>
                            <p>Custom IT Solutions Firm</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class=""><hr><hr><hr><hr></div>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-6"> 
                    <h1>Download App</h1>
                    <p>Wisdom new and valley answer. Contented it so is discourse recommend. Man its upon him call mile. An pasture he himself believe ferrars besides cottage.</p>
                    <div class="d-flex">
                        <img src="./img/google play.png" alt="">
                        <img src="./img/google play.png" alt="">
                    </div>
                </div>
                <div class="col-md-6"> 
                    <img src="./img/download.png" alt="download">
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-2">  </div>
                <div class="col-md-8"> 
                    <h1>Subscribe to get updated</h1>
                    <p>At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque.</p>
                </div>
                <div class="col-md-2"> 
                    <button class="text-capitalize">get start</button>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class=""></div>
                </div>
            </div>
        </div>
    </section>
</template>

<style scoped>

</style>
